...
SE  = chr_py3(240)  # Subnegotiation End
NOP = chr_py3(241)  # No Operation
DM  = chr_py3(242)  # Data Mark
BRK = chr_py3(243)  # Break
IP  = chr_py3(244)  # Interrupt process
AO  = chr_py3(245)  # Abort output
AYT = chr_py3(246)  # Are You There
EC  = chr_py3(247)  # Erase Character
EL  = chr_py3(248)  # Erase Line
GA  = chr_py3(249)  # Go Ahead
SB =  chr_py3(250)  # Subnegotiation Begin

BINARY = chr_py3(0)  # 8-bit data path
ECHO = chr_py3(1)  # echo
RCP = chr_py3(2)  # prepare to reconnect
SGA = chr_py3(3)  # suppress go ahead
NAMS = chr_py3(4)  # approximate message size
STATUS = chr_py3(5)  # give status
TM = chr_py3(6)  # timing mark
RCTE = chr_py3(7)  # remote controlled transmission and echo
NAOL = chr_py3(8)  # negotiate about output line width
NAOP = chr_py3(9)  # negotiate about output page size
NAOCRD = chr_py3(10)  # negotiate about CR disposition
NAOHTS = chr_py3(11)  # negotiate about horizontal tabstops
NAOHTD = chr_py3(12)  # negotiate about horizontal tab disposition
NAOFFD = chr_py3(13)  # negotiate about formfeed disposition
NAOVTS = chr_py3(14)  # negotiate about vertical tab stops
NAOVTD = chr_py3(15)  # negotiate about vertical tab disposition
NAOLFD = chr_py3(16)  # negotiate about output LF disposition
XASCII = chr_py3(17)  # extended ascii character set
LOGOUT = chr_py3(18)  # force logout
BM = chr_py3(19)  # byte macro
DET = chr_py3(20)  # data entry terminal
SUPDUP = chr_py3(21)  # supdup protocol
SUPDUPOUTPUT = chr_py3(22)  # supdup output
SNDLOC = chr_py3(23)  # send location
TTYPE = chr_py3(24)  # terminal type
EOR = chr_py3(25)  # end or record
TUID = chr_py3(26)  # TACACS user identification
OUTMRK = chr_py3(27)  # output marking
TTYLOC = chr_py3(28)  # terminal location number
VT3270REGIME = chr_py3(29)  # 3270 regime
X3PAD = chr_py3(30)  # X.3 PAD
NAWS = chr_py3(31)  # window size
TSPEED = chr_py3(32)  # terminal speed
LFLOW = chr_py3(33)  # remote flow control
LINEMODE = chr_py3(34)  # Linemode option
XDISPLOC = chr_py3(35)  # X Display Location
OLD_ENVIRON = chr_py3(36)  # Old - Environment variables
AUTHENTICATION = chr_py3(37)  # Authenticate
ENCRYPT = chr_py3(38)  # Encryption option
NEW_ENVIRON = chr_py3(39)  # New - Environment variables
# the following ones come from
# http://www.iana.org/assignments/telnet-options
# Unfortunately, that document does not assign identifiers
# to all of them, so we are making them up
TN3270E = chr_py3(40)  # TN3270E
XAUTH = chr_py3(41)  # XAUTH
CHARSET = chr_py3(42)  # CHARSET
RSP = chr_py3(43)  # Telnet Remote Serial Port
COM_PORT_OPTION = chr_py3(44)  # Com Port Control Option
SUPPRESS_LOCAL_ECHO = chr_py3(45)  # Telnet Suppress Local Echo
TLS = chr_py3(46)  # Telnet Start TLS
KERMIT = chr_py3(47)  # KERMIT
SEND_URL = chr_py3(48)  # SEND-URL
FORWARD_X = chr_py3(49)  # FORWARD_X
PRAGMA_LOGON = chr_py3(138)  # TELOPT PRAGMA LOGON
SSPI_LOGON = chr_py3(139)  # TELOPT SSPI LOGON
PRAGMA_HEARTBEAT = chr_py3(140)  # TELOPT PRAGMA HEARTBEAT
EXOPL = chr_py3(255)  # Extended-Options-List
NOOPT = chr_py3(0)
...
class InputBashLike(object):
    ...
    def process(self, line):
        ...
        if not self.complete:
            # Ask for more.
            self.process(self.handler.readline(prompt=self.handler.CONTINUE_PROMPT, echo=True))
...
# --------------------------- Environment Setup ----------------------------
    ...
    def __init__(self, request, client_address, server):
        ...
        # Track any asynchronous events registered with the timer command
        self.timer_events = list()
                    
        socketserver.BaseRequestHandler.__init__(self, request, client_address, server)

    class _FalseRequest(object):
        def __init__(self):
            self.sock = None
    ...
    def streamserver_handle(cls, sock, address):
        ...
        request = cls._FalseRequest()
        request._sock = sock
        server = None
        log.debug("Accepted connection, starting telnet session for {}.".format(address))
        try:
    ...
    def setterm(self, term):
        ...
        log.debug("Setting term type to %s" % (term, ))
        curses.initscr()
        ...
        if cmd == NOP:
            ...
            if subreq[0] == TTYPE and subreq[1] == IS:
                try:
                    ...
                    log.exception("Terminal type not known")
            elif subreq[0] == NAWS:
                # FIXME: check setnaws attribute
        ...
        while True:
            ...
            if c == theNULL:
                ...
                if echo is False:
                    ...
                    log.debug('readline: %s(hidden text)', bytes_to_str(prompt))
                else:
                    log.debug('readline: %s%r', bytes_to_str(prompt), bytes_to_str(result))
                return result
            elif c == curses.KEY_BACKSPACE or c == chr_py3(127) or c == chr_py3(8):
                if insptr > 0:
...
# --------------------------- Output Functions -----------------------------
    ...
    def writemessage(self, text):
        ...
        self.write(self._current_prompt + b''.join([chr_py3(i) for i in self._current_line]))
...
# ----------------------- Command Line Processor Engine --------------------
    ...
    def authentication_ok(self):
        ...
        if self.authCallback:
            if self.authNeedUser:
                username = self.readline(prompt=str_to_bytes(self.PROMPT_USER), use_history=False)
            if self.authNeedPass:
                password = self.readline(echo=False, prompt=str_to_bytes(self.PROMPT_PASS), use_history=False)
                if self.DOECHO:
                    self.write(b"\n")
            try:
    ...
    def handle(self):
        ...
        self.session_start()
        while self.RUNSHELL:
            raw_input = self.readline(prompt=str_to_bytes(self.PROMPT), echo=True)
